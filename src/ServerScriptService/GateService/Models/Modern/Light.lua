...
function handler.Instantiate(nodes)
	...
	model.LogicalState = { Light = Color3.new() }
	return model
end
...
function handler.Trigger(model, action)
	if action and action.Source == "Light" and action.Color then
		model.LogicalState.Light = action.Color
	end
end

function handler.ApplyLogicalState(model, state)
	model.Instance.Top.Color = state.Light
	model.Instance.Top.PointLight.Color = state.Light
	model.Instance.Top.PointLight.Enabled = state.Light ~= Color3.new()
end
