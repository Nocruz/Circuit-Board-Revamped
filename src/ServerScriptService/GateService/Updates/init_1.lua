...
local function drainDepthQueuesDFS()
    local steps = 0
    while MaxDepth >= 0 do
...
-- Drain depth queues (round-robin across depths, deepest-first preference)
local function drainDepthQueuesBFS()
    local steps = 0
    ...
    -- recompute MaxDepth (optional cleanup)
    local newMax = -1
    for d = 0, MaxDepth do
        if DepthQueues[d] and #DepthQueues[d] > 0 then
