return {
	...
	Process = function(self)
		...
		local hasTensConnection, tens = self:ReadAttributeFromNode("Tens", "Tens")
		tens = if hasTensConnection then tens.AsNumber() else tens
		...
		local roundedValue =
			if strategy == "Floor" then math.floor(scaledInput)
			elseif strategy == "Ceil" then math.ceil(scaledInput)
			else math.round(scaledInput)
