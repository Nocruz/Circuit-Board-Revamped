...
local function propagateFromOutput(gateID, outputName)
    local outConnections = Connections.GetAllOutgoing(gateID, outputName)
    for downstreamGateID, _ in pairs(outConnections) do
...
local function processGate(gateID, payload)
    ...
    if not gate or not gate.Specification.Process then return end
    ...
    -- Execute Logic
    local ok, err = pcall(gate.Specification.Process, gate, payload)
    ...
    if gate.Nodes and gate.Nodes.Signals then
        ...
        for outputName, currentSignal in pairs(gate.Nodes.Signals) do
            ...
            if previousSignals[outputName] ~= currentSignal then
                ...
                local connections = Connections.GetAllOutgoing(gateID, outputName)
                for inGate, connection in pairs(connections) do
                    for inNode, wire in pairs(connection) do
        ...
        local displayGui = (gate.Model) and (gate.Model:FindFirstChild("DisplayNameGui", true))
        if displayGui then
