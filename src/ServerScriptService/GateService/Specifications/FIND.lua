return {
  ...
  Process = function(self)
    ...
    local hasConnection, phrase = self:ReadAttribute("Phrase", "Phrase")
    phrase = if hasConnection then phrase.AsString() else phrase
    local hasConnection, index = self:ReadAttribute("Index", "Index")
    index = math.floor(if hasConnection then index.AsNumber() else index)
